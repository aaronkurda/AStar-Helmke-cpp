...
private:
	LogPlain& operator=(const LogPlain& p); // call forbidden
};
...
// HHe 23.03.05 use of error_output_enabled() added
#  define LOGTRACE_ERROR(msg)                         \
   if (g_init ) {               \
    gp_trace->mark(); (*gp_trace) << msg;             \
...
#endif
#define LT_ERROR(msg) LOGTRACE_ERROR(msg) /* HHe 19.09.03 */
...
/** \def COND_SET_ID(cond,id)
    This macro can be used to change the list of active
    IDs. If the condition (1st parameter) is evaluated to true, the
    specified ID is added. Changing the ID list makes sense only in
    debug level >= 2 (because below level 2 the output does not depend
    on the list of IDs). */
/** \def COND_DEL_ID(cond,id)
    This macro can be used to change the list of active
    IDs. If the condition (1st parameter) is evaluated to true, the
    specified ID is removed from the list.
	Changing the ID list makes sense only in
    debug level >= 2 (because below level 2 the output does not depend
    on the list of IDs). */
/** \def COND_READ_IDS(cond,file)
    This macro can be used to change the list of active
    IDs. If the condition (1st parameter) is evaluated to true,
	the content of the specified ID file added. Changing the ID list makes sense only in
    debug level >= 2 (because below level 2 the output does not depend
    on the list of IDs). */
#if (LOGTRACE_LEVEL>=2)
...
#  define INVARIANT_THROW(b,Excep,msg)                                    \
   if ((!(b))&&g_init) {                                                  \
	...
	LogViolation VLOG(LOGTRACE_VIOLATION_TAG,!(b),                         \
         "violation"#b,__FILE__, __LINE__);                               \
      VLOG.usage();  set_error_to_cerr(true); throw e; }
...
/** \def INVARIANT_THROW_RELEASE
    \brief Same as INVARIANT_THROW, except that nothing happens in debug level<=0
*/
#if (LOGTRACE_LEVEL>=1) 
...
/** \def DISABLE_CERR_OUTPUT
    \brief Output of ERROR, WARNING, INFO and INVARIANT is disabled now, in debug level<=1 nothing happens
*/
#if (LOGTRACE_LEVEL>=2)
#  define DISABLE_CERR_OUTPUT set_error_to_cerr(false);  
#else
#  define DISABLE_CERR_OUTPUT
#endif

/** \def ENABLE_CERR_OUTPUT
    \brief Output of ERROR, WARNING INFO and INVARIANT is enabled now, in debug level<=1 nothing happens
*/
#if (LOGTRACE_LEVEL>=2)
#  define ENABLE_CERR_OUTPUT set_error_to_cerr(true); 
#else
#  define ENABLE_CERR_OUTPUT
#endif

/** \def SUPPRESS_CERR_OUTPUT
    \brief Output of ERROR, WARNING and INVARIANT is disabled for statements st and 
	then enabled again. 
	\attention
	If an exception occurs during the execution of statements st
	then, however, the output will be still disabled.
*/
#define SUPPRESS_CERR_OUTPUT(st) { DISABLE_CERR_OUTPUT; st; ENABLE_CERR_OUTPUT}


/** \def SCONSTR_DESTR_CNT_OK 
    \brief boolean expression whether the number of destructor is equal to number
       constructor counts
*/
#if (LOGTRACE_LEVEL==3)
#   define CONSTR_DESTR_CNT_OK  (gp_idmap->constrEqualDestr())
#else
#   define CONSTR_DESTR_CNT_OK  (true)
#endif
